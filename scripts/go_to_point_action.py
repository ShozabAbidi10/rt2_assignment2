...
# ° /go_to_point
#
# This node recieves the desired goal coordinates as an action service '/go_to_point' request from the 'state_machine' node. Based on the recieved goal coordinates it     # computes the required linear and angular velocities of the robot to reach the target position and then publishes it in 'cmd_vel' topic. Since "/go_to_point" is an action   # service therefore, it offers the option to cancel the goal at any point during the execution. And for this purpose the goal preempt requested scheme has also been       # implemented which gets activated when user cancel any goal. Within this scheme number of cancel goals are also accumulated.
#
...
position_ = 0
##  A global variable with datatype 'int' used for storing the linear speed factor.
speed_factor_linear_ = 0
##  A global variable with datatype 'int' used for storing the angular speed factor.
speed_factor_angular_ = 0
##  A global variable used for initializing the publisher for topic '/cmd_vel'.
pub_ = None
##  A global variable used for initializing the publisher for topic '/success_goals'.
pub_succ_status_ = None
##  A global variable used for initializing the publisher for topic 'cancel_goals'.
pub_cancel_status_ = None
##  A global variable used for initializing the publisher for topic '/task_time'.
pub_task_time_ = None
##  A global variable with datatype 'int' for storing the total number of successful goals.
no_successful_target_ = 0
##  A global variable with datatype 'int' for storing the total number of cancelled  goals.
...
# \param msg arguement with data structure type 'Twist '  
# \return [none]
#
...
# \param state arguement with data type 'int '  
# \return [none].
#
# This function takes the arguement value and assigned it to the global variable 'state_'. 
#
...
def go_straight_ahead(des_pos):
    ...
    if err_pos > dist_precision_:
        twist_msg = Twist()
        twist_msg.linear.x = 0.3 + 0.3*speed_factor_linear_  # Here I can have to multply the  control speed value
        if twist_msg.linear.x > ub_d:
            twist_msg.linear.x = ub_d

        twist_msg.angular.z = kp_a*err_yaw
        pub_.publish(twist_msg)   # I make a change here.
...
# \param des_yaw arguement with datatype 'float'.  
# \return [none].
#
...
class PositionAction(object):
	...
	# \param name arguement of type 'String'  
	# \return [none].
	#
	...
	# \param goal arguement with custom define data structure type 'Position'. 
	# \return trure always.	
	#
